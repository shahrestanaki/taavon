	...
	styleUI : {
		...
		BootstrapMy : {/*MSH*****/
			common : {
				...
				content : "",
				deleteMsg : "نسبت به حذف مورد انتخابی اطمینان دارید ؟",
			},
			base : {
				entrieBox : "",
				viewBox : "table-responsive",
				headerTable : "table table-bordered  table-striped ",
				headerBox : "",
			...
			modal : {
				modal : "modal-content-my modal.show  modal-dialog",
				header : "bg-primary",
				headerWarn : "bg-primary",
				headerError : "bg-danger",
				title : "modal-title",
				content :"modal-body",
				resizable : "ui-resizable-handle ui-resizable-se",
			...
			navigator : {
				...
				icon_view_nav : "fa-sign-in",
				icon_newbutton_nav : "glyphicon-new-window",
				leftNavClass : "d-none d-sm-block",
				centerNavClass : "d-none d-sm-block",
				rigthNavClass : "d-none d-sm-block",
			},
			grouping : {
...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
		...
		formatCol = function (pos, rowInd, tv, rawObject, rowId, rdata){
			...
			result += " class=\"cell_"+nm+rowId+"\" aria-describedby=\""+ts.p.id+"_"+nm+"\"";
			return result + acp[0];
		},
		...
		setPager = function (pgid, tp){
			...
			$("#"+$.jgrid.jqID(pgid) )
			.append("<div id='"+pgcnt+"' class='ui-pager-control' role='group'><table " + getstyle(stylemodule, 'pagerTable', false, 'ui-pg-table ui-common-table ui-pager-table') + "><tbody><tr><td id='"+lft+"' align='left' class='"+$.jgrid.styleUI[ts.p.styleUI].navigator.leftNavClass+"'></td><td id='"+cent+"' align='center' style='white-space:pre;'></td><td id='"+rgt+"' align='right' class='"+$.jgrid.styleUI[ts.p.styleUI].navigator.rigthNavClass+"'></td></tr></tbody></table></div>") /****MSH*****/
			.attr("dir","rtl"); //explicit setting /***MSH***/
		...
		buildSearchBox = function (index, top, left, parent) {
			...
			// buttons
			str1 = "<div>";
			str1 +="<div class='search_buttons'><a tabindex='0' id='bs_reset' class='fm-button " + common.button.reset +" ui-reset'>"+texts.Reset+"</a></div>";
			str1 +="<div class='search_buttons'><a tabindex='0' id='bs_search' class='fm-button " + common.button.search + " ui-search'>"+texts.Find+"</a></div>";
			str1 += "</div>";
			elem.append(str1);
		...
		var thead = "<thead><tr class='ui-jqgrid-labels btn-primary ' role='row'>",/***MSH***/
		tdc, idn, w, res, sort ="",
...
//Helper functions
	...
	createModal : function(aIDs, content, p, insertSelector, posSelector, appendsel, css) {
		...
		mw.id = aIDs.themodal;
		var idType = aIDs.modalcontent;
		var headerType = classes.headerWarn;/****MSH***/
		if(idType.indexOf('del') == 0){
			headerType = "bg-danger";
		}else if (idType.indexOf('edit') == 0){
			headerType = "bg-primary";
		}else if (idType.indexOf('search') == 0){
			headerType = "bg-primary";
		}else if (idType.indexOf('alert') == 0){
			headerType = "bg-warning";
		}
		var mh = document.createElement('div');
		mh.className = "ui-jqdialog-titlebar " + headerType;/****MSH***/
		...
		// just in case
			coord.left = window.screen.width/2;// for alert and del message  parseInt(rtlt,10);/*MSH****coord.left = parseInt(coord.left,10) + parseInt(rtlt,10);*/
		}
		coord.left = window.screen.width/2 - 150; /*MSH****/
	...
	info_dialog : function(caption, content,c_b, modalopt) {
		...
		if(mopt.buttons.length > 0) {
			for(i=0;i<mopt.buttons.length;i++) {
				...
				buttstr += "<a id='"+mopt.buttons[i].id+"' class='fm-button " + common.button+" 11111'>"+mopt.buttons[i].text+"</a>";
			}
		}
		...
		cnt += c_b ? "<div class='" + classes.content + "' style='text-align:"+mopt.buttonalign+";padding-bottom:0.8em;padding-top:0.5em;background-image: none;border-width: 1px 0 0 0;'><a id='closedialog' class='fm-button " + common.button + " 2222'>"+c_b+"</a>"+buttstr+"</div>" :
...
// Date Validation Javascript
	...
	checkValues : function(val, valref, customobject, nam) {
		...
		if(edtrul) {
			...
			if(edtrul.maxLength !== undefined && !isNaN(edtrul.maxLength)) {
				if (val.toString().length > parseInt(edtrul.maxLength) ) { return [false,nm+": "+msg.maxValue+" "+edtrul.maxLength,""];}
			}			
			var filter;
	...
	buildButtons : function ( buttons, source, commonstyle) {
		...
		$.each(buttons, function(i,n) {
			...
			str = "<a  data-index='"+i+"' id='" + n.id + "' class='9999999 fm-button " + commonstyle.button + icon + n.text+"</a>";
			if(n.position === "last" ) {
				source = source + str;
			} else {
				source = str + source;
		...
		*/
		...
		this.createTableForGroup = function(group, parentgroup) {
			...
			if(this.p.groupButton) {
				inputAddSubgroup = $("<input type='button' value='+ {}' title='" +that.p.subgroup+"' class='add-group " + common.button + " 333333'/>");
			...
			var inputAddRule = $("<input type='button' value='+' title='"+that.p.addrule+"' class='add-rule ui-add " + common.button + " 44444'/>"), cm;
			...
			if (parentgroup !== null) { // ignore the first group
				var inputDeleteGroup = $("<input type='button' value='-' title='"+that.p.delgroup+"' class='delete-group " + common.button + " 55555'/>");
		...
		this.createTableRowForRule = function(rule, group ) {
			...
			ruleFieldSelect.on('change',function() {
				...
				/*for (i = 0; i < that.p.columns.length; i++) {
					if(that.p.columns[i].name === rule.field){
						cm.searchoptions.name = that.p.columns[i].searchName;
					}
				}*/
				...
				for ( i = 0 ; i < op.length; i++) {
					...
					if(ina !== -1) {
						...
						s += "<option value='"+that.p.ops[ina].oper+" 2'>"+that.p.ops[ina].text+"</option>";
						so++;
					}
				}
			...
			if(this.p.ruleButtons === true) {
			var ruleDeleteInput = $("<input type='button' value='-' title='"+that.p.delrule+"' class='delete-rule ui-del " + common.button + " 66666'/>");
	...
	searchGrid : function (p) {
		...
		return this.each(function() {
			...
			if ( $("#"+$.jgrid.jqID(IDs.themodal))[0] !== undefined ) {
				...
				bS  ="<a id='"+fid+"_search' class='fm-button " + common.button.find + " fm-button-icon-right ui-search'><span class='" + common.icon_base + " " +classes.icon_search + "'></span>"+p.Find+"</a>",
				bC  ="<a id='"+fid+"_reset' class='fm-button " + common.button.reset +" fm-button-icon-left ui-reset'><span class='" + common.icon_base + " " +classes.icon_reset + "'></span>"+p.Reset+"</a>",
...
$.jgrid.extend({
	editGridRow : function(rowid, p){
		...
		return this.each(function(){
				...
				if(ret[0] && !rp_ge[$t.p.id].processing) {
					...
					var ajaxOptions = $.extend({
						...
						if(data.status === undefined || _statusError.indexOf(data.status) != -1){/******MSH****/
								switch(data.status) {
									case 200:
										if(ajaxOptions.type =="DELETE"){
											showMsg('عملیات حذف','عملیات حذف با موفقیت انجام گرفت','S',2);
										}else{
									...
									case 422 :
										...
										break;										
									case 500:
										if(Msh_ContainUnicodePatern(data.responseText) == true){
											showMsg('خطا',data.responseText,'D');
										}else{
							...
							}else if(data.status >= 300 && data.status !== 304) {
								ret[0] = false;
			...
			bS  ="<a id='sData' class='fm-button " + commonstyle.button.save + "'>"+p.bSubmit+"</a>",
			bC  ="<a id='cData' class='fm-button " + commonstyle.button.cancel + "'>"+p.bCancel+"</a>",
	...
	delGridRow : function(rowids,p) {
		...
		return this.each(function(){
			...
			if ( $("#"+$.jgrid.jqID(IDs.themodal))[0] !== undefined ) {
				...
				tbl += "<tr><td class=\"delmsg\" style=\"white-space:pre;\">"+commonstyle.deleteMsg+"</td></tr><tr><td >&#160;</td></tr>";/***MSH****/
				// buttons at footer
				tbl += "</tbody></table></div>";
				var bS  = "<a id='dData' class='fm-button " + commonstyle.button.delete + "'>"+p.bSubmit+"  </a>",
				bC  = "<a id='eData' class='fm-button " + commonstyle.button.cancel + "'> "+p.bCancel+"  </a>",
				user_buttons = ( $.isArray( rp_ge[$t.p.id].buttons ) ? $.jgrid.buildButtons( rp_ge[$t.p.id].buttons, bS + bC, commonstyle ) : bS + bC );
						...
						var ajaxOptions = $.extend({
							...
							complete:function(data,status){
								...
								/*}else if (data.status == 200){
									showMsg('عملیات حذف','حذف دیتا با موفقیت انجام گرفت','S',1);*/
								}else if(data.status >= 300 && data.status !== 304) {
									ret[0] = false;
		...
		return this.each(function() {
			...
			if ($("#"+alertIDs.themodal)[0] === undefined) {
				if(!o.alerttop && !o.alertleft) {
					...
					if(o.alertleft > window.screen.width){/****MSH*****/
						o.alertleft =  window.screen.width / 2 ;
					}
				}
